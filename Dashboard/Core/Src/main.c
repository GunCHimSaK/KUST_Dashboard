...
static void MX_LPTIM2_Init(void)
{
	...
	LPTIM_OC_ConfigTypeDef sConfig1 =
	{ 0 };
	...
	if (HAL_LPTIM_OC_ConfigChannel(&hlptim2, &sConfig1, LPTIM_CHANNEL_1)
			!= HAL_OK)
	{
		Error_Handler();
	}
...
static void MX_GPIO_Init(void)
{
	...
	/*Configure GPIO pins : STM_DIN0_Pin STM_DIN1_Pin STM_Button_6_Pin STM_Button_5_Pin
	 STM_Button_4_Pin */
	GPIO_InitStruct.Pin = STM_DIN0_Pin | STM_DIN1_Pin | STM_Button_6_Pin
			| STM_Button_5_Pin | STM_Button_4_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
